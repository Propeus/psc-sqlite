...
```powershell
. .\psc-sqlite.ps1
$database = [Sqlite]::new("database.sqlite") #Cria no mesmo caminho onde está a instancia do PowerShell
$database.ExecuteNonQuery('CREATE TABLE IF NOT EXISTS TEST( ID INT NOT NULL, NME TEXT NOT NULL)');
```
...
$database.CreateTableFromType([TEST])
...
$database.CreateTableFromType([TEST],$true,$false,$false)
...
$database.CreateTableFromType([TEST],$true,$true,$false)

# INFO
# Cria uma tabela baseado no tipo especificado caso não exista.
$database.CreateTableFromType([TEST],$true,$false,$true)

# INFO
# Cria uma tabela caso nao exista.
$database.CreateTableIfNotExistsFromType([TEST])
...
$database.RecreateTableFromType([TEST])
```
...
$database.InsertFromType($typeTeste)
